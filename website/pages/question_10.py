...
def result_barcharts(dataframe, column_box_office):
    ...
    success_data = (
        dataframe.groupby("Used_Plotphrases")[column_box_office].mean().reset_index()
    )
    ...
    figure_sucess = px.bar(
        ...
        labels={
            "Used_Plotphrases": "Plotphrase used?",
            "Average_Success": "Average Box office in $",
        },
        color_discrete_map={"True": "#66b3ff", "False": "#ff9999"},
        category_orders={"Used_Plotphrases": ["True", "False"]},
    )
...
figure_boxplot_fantasy = boxplot(
    livedata_fantasy, column_box_office="box_office_worldwide", genre="Fantasy"
)
figure_bar_count_fantasy, figure_bar_sucess_fantasy = result_barcharts(
    livedata_fantasy, column_box_office="box_office_worldwide"
)
    ...
    [
        ...
        dcc.Loading(
            ...
            children=html.Div(
                [
                    dcc.Graph(
                        id="box_office_plot", figure=figure_boxplot_fantasy  # dfault
                    )
                ]
            ),
        ),
...
def run_analysis(n_clicks, threshold_value, choosen_genre):
    ...
    updated_dataframe["Used_Plotphrases"] = (
        updated_dataframe["Similarity_Score"] >= threshold_value
    ).map({True: "True", False: "False"})

    figure_count, figure_sucess = result_barcharts(
        updated_dataframe, "box_office_worldwide"
    )
    figure_boxplot = boxplot(
        updated_dataframe, column_box_office="box_office_worldwide", genre=shown_titel
    )
