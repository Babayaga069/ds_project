...
def pie_chart(dataframe, columname="genres", topgenres=5, filter_zero=True, year=""):
    ...
    fig.update_traces(rotation=140, textinfo="percent", texttemplate="%{percent:.1%}")
...
def plot_growth(
    ...
    # Merge
    dataframe_compare = pd.DataFrame(
        {f"Year_{year_old}": count_old, f"Year_{year_new}": count_new}
    ).fillna(0)

    # filter out unimporantant genre
    dataframe_compare = dataframe_compare[
        (dataframe_compare[f"Year_{year_old}"] + dataframe_compare[f"Year_{year_new}"])
        >= 0.01
    ]

    dataframe_compare["Growth (%)"] = np.where(
        dataframe_compare[f"Year_{year_old}"] == 0,
        100.0,
        (
            (
                dataframe_compare[f"Year_{year_new}"]
                - dataframe_compare[f"Year_{year_old}"]
            )
            / dataframe_compare[f"Year_{year_old}"]
        )
        * 100,
    ...
    colorbar = [
        "#d62728" if x < 0 else "#2ca02c" for x in dataframe_compare["Growth (%)"]
    ]

    fig = px.bar(
        dataframe_compare, x="Growth (%)", y=dataframe_compare.index, orientation="h"
    )
...
def plot_genre_comp(
    ...
    box_old = (
        dataframe_old_filterd.groupby(column)[box_office].sum()
        * inflation_factor
        / 1000000
    )
    box_new = dataframe_new_filterd.groupby(column)[box_office].sum() / 1000000

    dataframe_revenue = pd.DataFrame(
        {f"{year_old}": box_old, f"{year_new}": box_new}
    ).fillna(0)
...
layout = html.Div(
    [
        ...
        html.Div(
            [dcc.Graph(figure=fig_growth)], style={"width": "50%", "marginTop": "50px"}
        ),
        html.Div(
            [
                ...
                dcc.Graph(
                    id="box-office-graph", style={"height": "700px", "width": "200%"}
                ),
            ],
...
def update_pie_charts(choosen_top_genres):
    ...
    fig_pie_2000 = pie_chart(
        dataframe=data_2000,
        topgenres=choosen_top_genres,
        filter_zero=True,
        year="2000-2005",
    )
    fig_pie_2025 = pie_chart(
        dataframe=data_2025,
        topgenres=choosen_top_genres,
        filter_zero=True,
        year="2020-2025",
...
def update_box_office_chart(select, choosen_genre):
    ...
    if select == "total":
        ...
        return plot_genre_comp(
            dataframe_old=data_2000, dataframe_new=data_2025, genre_list=choosen_genre
        )
